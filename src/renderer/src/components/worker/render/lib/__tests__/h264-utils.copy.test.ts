...
const { RawBitstream } = require('../h264-utils')

const nalu = new Uint8Array([1, 2, 0, 0, 3, 0, 4, 0, 0, 3, 0, 5, 11, 12, 13, 14, 15, 0, 0])

const deemNalu = new Uint8Array([1, 2, 0, 0, 0, 4, 0, 0, 0, 5, 11, 12, 13, 14, 15, 0, 0])

const nalu2 = new Uint8Array([
  39, 100, 0, 40, 172, 43, 64, 176, 75, 66, 0, 0, 3, 0, 2, 0, 0, 3, 0, 121, 193, 64, 7, 161, 0, 0,
  190, 188, 222, 247, 0, 241, 34, 106
])

const deemNalu2 = new Uint8Array([
  39, 100, 0, 40, 172, 43, 64, 176, 75, 66, 0, 0, 0, 2, 0, 0, 0, 121, 193, 64, 7, 161, 0, 0, 190,
...
test('copy bits from one to another', () => {
  ...
  expect(source.stream).toEqual(nalu)
  expect(target.stream).toEqual(nalu)
})
...
test('copy bits from one to another, multiple chunks', () => {
  ...
  expect(source.stream).toEqual(nalu2)
  expect(target.stream).toEqual(nalu2)
})
